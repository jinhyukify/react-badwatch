...
class BoardBox extends React.Component{
	...
	_setPageNumbers(){
		...
		var tempPageNumbers = [];
		tempPageNumbers.push(<li onClick={this._before} key={-1}><a><i className="material-icons">chevron_left</i></a></li>);
		for(var i = this.state.startPageNumber ; i <= this.state.endPageNumber; i++){
			tempPageNumbers.push(<li className={`waves-effect ${this.state.currentPageNumber==i? 'active': ''}`} onClick={this._changePage.bind(this, i)} key={i}><a>{i}</a></li>);
		}
		tempPageNumbers.push(<li onClick={this._next} key={-2}><a><i className="material-icons">chevron_right</i></a></li>);
	...
	render(){
		return(
			<div className="board-top">
				<a className="waves-effect btn" onClick={this._changeBoardType.bind(this, 'free')}>자유게시판</a>
				<a className="waves-effect btn" onClick={this._changeBoardType.bind(this, 'strategy')}>전략게시판</a>
				<p/>
				<div>
